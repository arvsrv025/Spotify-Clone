  ...
  return (
    <>
        ...
        <div className='mt-10 flex gap-8 flex-col md:flex-row md:items-end'>
            ...
            <img className='w-48 rounded ' src={albumData.image} alt="image"></img>
            <div className='flex flex-col'>
            <p>Playlist</p>
            <h2 className='text-5xl font-bold mb-4 md:text-7xl'>{albumData.name}</h2>
            <h4>{albumData.desc}</h4>
            <p className='mt-1'>
                 <img className='inline-block w-5 ' src={assets.spotify_logo} alt="spotify_logo"></img>
                <b>Spotify</b>
                •  11,811,962 likes
                •  <b>  50 songs, </b>
                 about 2hr 30 mins
       ...
       <hr/>
        {
            songsData.map((item,index)=>{
                return <div onClick={()=>playWithId(item.id)} key={index} className='grid grid-cols-3 sm:grid-cols-4 gap-2 p-2 items-center text-[#a7a7a7] hover:bg-[#ffffff2b] cursor-pointer'>
                    ...
                    <p className='text-[15px] sm-hidden sm-block'>5 days ago</p>
